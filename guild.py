...
class ShiloGuild:
    ...
    async def Leave(self, ctx):
        ...
        if not _can_command(ctx):
            await ctx.send(
                f'You must connect yourself to the same channel as {ctx.bot.user.name}!'
            )
            return
    ...
    async def Start(self, ctx, playlist_name=None, restart=False):
        ...
        auto_name = playlist_name or (self._playlist.name if self._playlist else None)
        if not auto_name:
    ...
    async def Stop(self, ctx):
        ...
        print(f'[INFO] Playback of "{self._playlist.current_track_name}" stopped.')
    ...
    async def FastForward(self, ctx, interval_str):
        ...
        await ctx.send(f'Fast-forwarding "{self._playlist.current_track_name}".')
    ...
    async def List(self, ctx, playlist_name=None):
        ...
        if not playlist_name:
            ...
            await ctx.send(playlist.print_playlists(self._playlists, current_name))
            return
